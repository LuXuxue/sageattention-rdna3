...
>**说明：**
>在ComfyUI中，native的性能只有triton的90%（反映在it/s上）。
>可能是iGPU固有问题，也可能是代码编写方式的问题（本项目参考了comfy-kitchen的实现方式）。
>comfy-kitchen的native与triton在gfx1103上也有近似幅度的性能损失。
>因此为了获得最佳性能，默认采用triton，native仅用于探索。
...
int8 量化路径:
    D=64,  self-attn         -> (BM=64, BN=64)  [wpe1]
